    ...
    <div class="UserInfo">
        ...
        <div class="userInfomation" v-else>
            <section>
                ...
                <p>注册时间: {{$filters.formatDate(userinfo.create_at)}}</p>
            </section>
            <div class="replies">
                <p>回复的主题</p>
                <ul>
                    <li v-for="item of userinfo.recent_replies" :key="item.id">
                        <router-link :to="{name: 'article', params: {id: item.id, name: item.author.loginname}}" :title="item.title">
                            {{item.title}}
                        </router-link>
                    </li>
                </ul>
        ...
        getData() {
            ...
            .then((respond)=> {
                if (respond.data.success === true) {
                    this.userinfo = respond.data.data
                    this.loading = false
                }
            })
            .catch((err)=> {
